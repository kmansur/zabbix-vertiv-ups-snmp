...
write("VERSION", "1.5.2\n")
write("STABLE_VERSION", "1.5.2\n")

for path in ("templates/7.0/vertiv-by-snmp.yaml", "templates/8.0/vertiv-by-snmp.yaml"):
    replace(path, "      version: 1.5-1\n", "      version: 1.5-2\n")
    replace(
        path,
        "      License: MIT for the original work in this repository. See LICENSE and NOTICE.md for attribution and third-party licensing notes.\n",
        "      License: GNU GPL v3.0 only (GPL-3.0-only). Based on Template Vertiv by Mihguel da Silva Santos Tavares de Araujo and used with the original author's permission. See LICENSE and NOTICE.md.\n",
    )
    marker = (
        "      Version 1.5.0 hardens production monitoring with an explicit SNMP heartbeat, "
        "RFC1628 identification/test coverage, active-alarm table discovery, safe-by-default handling "
        "of unvalidated private metrics, standard battery current/temperature objects, and production "
        "CI import validation. Vendor-private event encodings remain unguessed.\n"
    )
    note = (
        marker
        + "\n\n      Version 1.5.2 aligns repository and template licensing/attribution with the original "
        "template author's GPLv3 authorization. Monitoring items, OIDs, triggers and read-only "
        "behavior are unchanged from 1.5.1.\n"
    )
    replace(path, marker, note)

# README EN
replace("README.md", "- **Latest stable release:** `1.5.1`", "- **Latest stable release:** `1.5.2`")
replace("README.md", "- **Current repository version:** `1.5.1`", "- **Current repository version:** `1.5.2`")
replace(
    "README.md",
...
# README PT-BR
replace("README.pt-BR.md", "- **Última versão estável:** `1.5.1`", "- **Última versão estável:** `1.5.2`")
replace("README.pt-BR.md", "- **Versão atual do repositório:** `1.5.1`", "- **Versão atual do repositório:** `1.5.2`")
replace(
    "README.pt-BR.md",
...
# Changelogs: promote current Unreleased licensing block to 1.5.2.
for path, published, validation in (
    (
        "CHANGELOG.md",
        "Released repository version: **1.5.2**.",
        "- CI, template/documentation/production validators, Zabbix 7.0 import/upgrade and CodeQL passed for the licensing change before release promotion.",
    ),
    (
        "CHANGELOG.pt-BR.md",
        "Versão publicada do repositório: **1.5.2**.",
        "- CI, validadores de template/documentação/produção, importação/upgrade no Zabbix 7.0 e CodeQL passaram para a alteração de licenciamento antes da promoção da release.",
    ),
):
    text = read(path)
    heading = "Unreleased" if path == "CHANGELOG.md" else "Não lançado"
    m = re.search(rf"## \[{re.escape(heading)}\]\n(.*?)(?=## \[1\.5\.1\])", text, re.S)
    if not m:
        raise RuntimeError(f"Unreleased block not found in {path}")
    body = m.group(1).strip()
    validated = "### Validated" if path == "CHANGELOG.md" else "### Validado"
    replacement = (
        f"## [{heading}]\n\n"
        f"## [1.5.2] - 2026-09-14\n\n{published}\n\n{body}\n\n"
        f"{validated}\n\n{validation}\n\n"
    )
    write(path, text[: m.start()] + replacement + text[m.end() :])
...
print("Release preparation changes applied.")
