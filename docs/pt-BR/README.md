...
- **MIB privada Vertiv/Liebert** — `1.3.6.1.4.1.476.1.42`, utilizada para status, identificação, bateria e métricas ambientais específicas do fabricante.
...
3. **Detalhes do fabricante quando úteis.** OIDs Vertiv adicionam status do sistema, autoteste, estado de carga, energia, ambiente e contadores de qualidade.
4. **Descoberta dinâmica de linhas.** As tabelas de entrada, saída e bypass são descobertas sem assumir previamente uma ou três fases.
5. **Padrões seguros.** Limites de triggers são expostos como macros e podem ser sobrescritos por template, grupo de hosts ou host.
6. **Manutenção rastreável.** Os exports Zabbix 7.0 e 8.0 são versionados separadamente e validados para manter equivalência semântica.
...
Após a primeira importação, compare os valores do Zabbix com o LCD/interface web do nobreak e revise os itens unsupported. Consulte [Troubleshooting](troubleshooting.md).
