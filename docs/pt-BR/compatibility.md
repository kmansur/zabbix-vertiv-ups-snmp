...
Esta matriz separa **compatibilidade de engenharia** de **homologação em campo**. Um export pode passar na validação estrutural/importação sem comprovar cada OID em todos os firmwares. A release 1.5.0 usa o comportamento real do equipamento para definir os defaults seguros, enquanto cenários controlados adicionais em campo continuam em andamento no hardware de referência.

| Nobreak / placa | Zabbix | Status | Observações |
| --- | --- | --- | --- |
| Vertiv `ITA-20k00AL3A02E00` (20 kVA), firmware UPS `V220` / placa `IS-UNITY-DP`, firmware `8.5.1.0` (`IS-UNITY_8.5.1.0_00173`) | 7.0.30 | 1.5.0 publicada; validação estendida em campo em andamento | Estados principais, valores elétricos, enums textuais e dashboard foram observados em hardware real. `upsBatteryCurrent` (`.1.3.6.1.2.1.33.1.2.6.0`) e `upsBatteryTemperature` (`.1.3.6.1.2.1.33.1.2.7.0`) retornam `noSuchObject` nesta placa/firmware. |
...
A placa respondeu como fabricante `Vertiv`, modelo `IS-UNITY-DP`, firmware `8.5.1.0`, build `IS-UNITY_8.5.1.0_00173` e `sysObjectID` `.1.3.6.1.4.1.476.1.42`. O nobreak reportou modelo `ITA-20k00AL3A02E00` e firmware `V220`. O ambiente de referência usa **Zabbix Server 7.0.30**. O `sysName.0` retornou `nobreak-01`, enquanto os objetos de nome do UPS retornaram preenchimento de zeros; por isso o inventory `NAME` usa `sysName.0` como fonte confiável. Números de série não são publicados nesta matriz.

## Resultado de campo — cobertura RFC1628

O walk completo da árvore `1.3.6.1.2.1.33` confirmou suporte funcional aos grupos padronizados de identificação, bateria parcial, entrada, saída, bypass, alarmes, resultados de teste e configuração. A release 1.5.0 usa apenas objetos de leitura; os grupos de controle/configuração presentes no agente não são usados para escrita.
...
A placa usada como equipamento de referência implementa apenas parte do grupo UPS-MIB de bateria. Foram observados:
...
Por isso, a release 1.5.0 mantém corrente e temperatura RFC1628 no template apenas como itens opcionais **desabilitados por padrão**. Eles não participam de dashboard nem triggers de produção e só devem ser habilitados em outra placa/firmware depois de confirmar suporte.
...
## Status da validação em campo

A 1.5.0 é uma release estável de software, mas a combinação de hardware de referência permanece em validação estendida em campo. Os checks controlados restantes de alarme, disponibilidade, transição de diagnóstico e aprovação do operador estão registrados no [registro de validação em campo da 1.5.0](homologation-1.5.0.md). Até a conclusão desses testes, esta matriz evita intencionalmente a expressão **homologada em campo** para o hardware de referência.
