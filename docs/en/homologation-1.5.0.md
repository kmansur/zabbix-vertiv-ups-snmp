# Field homologation record — candidate 1.5.0

[Português (Brasil)](../pt-BR/homologation-1.5.0.md)

**Candidate:** 1.5.0  
**Record date:** 2026-09-14  
**Overall status:** **IN PROGRESS / RELEASE BLOCKED**

This record separates evidence already collected on real hardware from checks that still require a controlled field action. A missing field check is never converted into PASS from repository/CI evidence alone.
...
| Template candidate | `1.5.0` / `vendor.version: 1.5-0` |
...
| RFC1628 battery current | COMPATIBILITY EXCEPTION | `upsBatteryCurrent` returned `noSuchObject`; candidate item remains disabled by default |
| RFC1628 battery temperature | COMPATIBILITY EXCEPTION | `upsBatteryTemperature` returned `noSuchObject`; candidate item remains disabled by default |
| Vertiv private battery current | PASS | Private OID ending `4149` observed working and retained in production dashboard |
...
| Zabbix 7.0 fresh import / upgrade | PASS (CI) | CI imports the stable baseline and upgrades to candidate through the Zabbix API |

## Mandatory checks still pending

The following checks require controlled access to the real monitoring environment and are **release blockers**:
...
- [ ] Record the SNMP version/security level used in the homologation environment.
- [ ] Confirm no unexpected unsupported items remain enabled by default on this exact card/firmware.
- [ ] Record final operator sign-off and homologation date.

## Promotion decision

**Current decision: DO NOT TAG `v1.5.0`.**

Promotion is allowed only after every mandatory checkbox above is complete or an explicit, documented non-blocking exception is approved. When the gate closes:

1. change this record to **PASS** and add the final homologation date/sign-off;
2. update the compatibility matrix to `1.5.0 field-homologated`;
3. date the 1.5.0 changelog entry;
4. update `STABLE_VERSION` to `1.5.0`;
5. create tag/release `v1.5.0` only after release CI succeeds.
