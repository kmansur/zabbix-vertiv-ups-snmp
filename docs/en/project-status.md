...
**Latest production release gate: PASS — maintainer approved 1.5.0 on 2026-09-14**

**Current repository candidate: 1.5.1 — global-dashboard tooling/documentation**
...
Repository candidate **1.5.1** adds the optional global-dashboard generator, bilingual deployment documentation and automated conversion/layout tests. It does not change item keys, macros, OIDs, triggers or the 1.5.0 monitoring semantics. The maintainer successfully validated the generator in `--dry-run` mode against a real Zabbix 7.0 environment.
...
| Global dashboard generator | VALIDATED CANDIDATE | 1.5.1 tooling; real Zabbix 7.0 `--dry-run` validated by maintainer |
...
The 1.5.1 candidate is not described as the latest stable release until the maintainer explicitly promotes it and the release/tag checks pass.
